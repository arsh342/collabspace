...
const validateMessage = [
  ...
  body("messageType")
    ...
    .withMessage("Message type must be one of: text, file, image, voice, system"),
];

// @route   GET /api/chat/conversations
// @desc    Get user's chat conversations (teams and direct messages)
// @access  Private
router.get("/conversations", authenticateSession, catchAsync(async (req, res) => {
  try {
...
// @access  Private
router.post("/messages", authenticateSession, validateMessage, catchAsync(async (req, res) => {
  try {
...
// @desc    Edit a message
// @access  Private
router.put("/messages/:messageId", authenticateSession, catchAsync(async (req, res) => {
  try {
...
// @access  Private
router.delete("/messages/:messageId", authenticateSession, catchAsync(async (req, res) => {
  try {
    const { messageId } = req.params;
...
// @access  Private
router.post("/messages/:messageId/reaction", authenticateSession, catchAsync(async (req, res) => {
  try {
    const { messageId } = req.params;
...
// @access  Private
router.get("/team/:teamId/members", authenticateSession, catchAsync(async (req, res) => {
  try {
    ...
    const members = team.members.map(member => ({
      ...
      email: member.email,
      avatar: member.avatar || `https://ui-avatars.com/api/?name=${member.username}&background=7B61FF&color=fff`,
      online: member.isOnline || false,
      lastSeen: member.lastSeen,
      role: member._id.toString() === team.owner._id.toString() ? 'admin' : 'member'
    }));
