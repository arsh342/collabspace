...
const mockRequireOrganiser = (req, res, next) => {
  if (req.user.role !== 'organiser') {
    return res.status(403).json({ success: false, message: 'Organiser access required' });
  }
  next();
};
...
describe('Teams Routes', () => {
  ...
  beforeEach(() => {
    ...
    mockRouter.post('/teams', mockAuthenticateSession, mockRequireOrganiser, (req, res) => {
      if (!req.body.name) {
    ...
    mockRouter.get('/teams/:id', mockAuthenticateSession, (req, res) => {
      ...
      res.json({ success: true, team: { _id: req.params.id, name: 'Test Team' } });
    });
    ...
    mockRouter.delete('/teams/:id', mockAuthenticateSession, mockRequireOrganiser, (req, res) => {
  ...
  describe('POST /api/teams', () => {
    it('should create team when user is organiser', async () => {
      ...
      orgRouter.post('/teams', (req, res, next) => {
        req.user = { _id: 'user123', role: 'organiser' };
        next();
      }, mockRequireOrganiser, (req, res) => {
        if (!req.body.name) {
          return res.status(400).json({ success: false, message: 'Team name is required' });
        }
        res.status(201).json({ success: true, team: { name: req.body.name } });
