  ...
  describe('OnlineUsersManager', () => {
    describe('addUser', () => {
      it('should add user to online status', async () => {
        ...
        expect(onlineUsers.addUser).toHaveBeenCalledWith('user123', 'socket456');
      });
  ...
  describe('RealTimeRateLimiter', () => {
    describe('checkLimit', () => {
      it('should allow action within limit', async () => {
        const allowed = await rateLimiter.checkLimit('user123', 'sendMessage', 10, 60);
        
        expect(allowed).toBe(true);
        expect(rateLimiter.checkLimit).toHaveBeenCalledWith('user123', 'sendMessage', 10, 60);
      });
  ...
  describe('MessageCache', () => {
    describe('addMessage', () => {
      it('should add message to cache', async () => {
        ...
        expect(result).toBe(true);
        expect(messageCache.addMessage).toHaveBeenCalledWith('room123', message);
      });

      it('should handle Redis unavailable', async () => {
        messageCache.addMessage.mockResolvedValue(false);
        
        const result = await messageCache.addMessage('room123', { text: 'test' });
  ...
  describe('NotificationQueue', () => {
    describe('queueNotification', () => {
      it('should queue notification for user', async () => {
        ...
        notificationQueue.queueNotification.mockResolvedValue('notification-id-123');
        
        const result = await notificationQueue.queueNotification('user123', notification);
        
        expect(typeof result).toBe('string'); // Should return notification ID
        expect(notificationQueue.queueNotification).toHaveBeenCalledWith('user123', notification);
      });

      it('should handle Redis unavailable', async () => {
        notificationQueue.queueNotification.mockResolvedValue(false);
        
        const result = await notificationQueue.queueNotification('user123', { type: 'test' });
    ...
    describe('getUserNotifications', () => {
      it('should return user notifications', async () => {
        ...
        notificationQueue.getUserNotifications.mockResolvedValue(mockNotifications);
        
        const notifications = await notificationQueue.getUserNotifications('user123', 20);
      ...
      it('should return empty array when Redis unavailable', async () => {
        ...
        const notifications = await notificationQueue.getUserNotifications('user123');
